...
void PacketBundleDecoder::ProcessHDLPacket(unsigned char* data, unsigned int data_length)
{
  ...
  for (int i = 0; i < HDL_FIRING_PER_PKT; ++i)
  {
    ...
    for (int j = 0; j < HDL_LASER_PER_FIRING; j++)
    {
      ...
      {
        PushFiringData(laserId, firingData.rotationalPosition, dataPacket->gpsTimestamp, firingData.laserReturns[j],
...
void PacketBundleDecoder::PushFiringData(unsigned char laserId, unsigned short azimuth, unsigned int timestamp,
        ...
        if (px.first == "px")
        {
          ...
          if (index != -1)
          {
            ...
            laser_corrections_[index].cosVertCorrection =
                std::cos(HDL_Grabber_toRadians(laser_corrections_[index].verticalCorrection));
            laser_corrections_[index].sinVertCorrection =
                std::sin(HDL_Grabber_toRadians(laser_corrections_[index].verticalCorrection));
          }
        }
      }
    }
...
{
  double hdl32VerticalCorrections[] = {-30.67, -9.3299999, -29.33, -8,         -28,    -6.6700001, -26.67, -5.3299999,
...
std::deque<PacketBundleDecoder::HDLFrame> PacketBundleDecoder::GetFrames() { return _frames; }

void PacketBundleDecoder::ClearFrames() { _frames.clear(); }
