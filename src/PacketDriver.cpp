...
PacketDriver::PacketDriver(unsigned int port)
    : _port(port)
{
...
bool PacketDriver::GetPacket(std::string* data, unsigned int* data_length)
{
  try
  {
    _socket->async_receive(boost::asio::buffer(_rx_buffer, 1500),
                           boost::bind(&PacketDriver::GetPacketCallback, this, boost::asio::placeholders::error,
                                       boost::asio::placeholders::bytes_transferred, data, data_length));
    _io_service.reset();
    _io_service.run();
    return (true);
  }
...
void PacketDriver::GetPacketCallback(const boost::system::error_code& error, std::size_t num_bytes, std::string* data,
